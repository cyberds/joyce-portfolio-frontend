...
function desktopLayout(): JourneyLayout {
  ...
  return { vbW, vbH, bias: 0.5, nodes, cards, pathD, cardOffset };
}

/**
 * Narrow: a single centred column. The card can't sit beside the spine on a
 * phone, so it drops underneath and the camera holds the head high in the frame
...
function mobileLayout(): JourneyLayout {
  ...
  const cards: Point[] = nodes.map((n) => ({ x: 300, y: n.y + 340 }));
  const cardOffset = nodes.map(() => ({ x: 0, y: 120 }));

  const last = nodes[nodes.length - 1];
  const vbH = last.y + TAIL + 60;
  const pathD = buildPath(nodes, { x: 300, y: 80 }, { x: 300, y: last.y + TAIL });

  return { vbW, vbH, bias: 0.3, nodes, cards, pathD, cardOffset };
}
